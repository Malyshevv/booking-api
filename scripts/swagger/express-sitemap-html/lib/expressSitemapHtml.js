...
const {router} = require("../../../../routes/Static/routerStatic");
...
function swaggerJson(swDocument, app) {
    ...
    let swaggerDocument = swDocument


    routes.forEach(r => {
        if (r || r.path) {
            var finishPath = '';
            var path = r.path.replaceAll('\\', '');
            path = path.replaceAll('/api', '/api/').split('/');

            path.forEach(p => {
                if (p.length && (p !== 'undefined' ||p !== undefined)) {
                    finishPath += `/${p}`;
                }
                r.path = finishPath;
            })
    ...
    swaggerDocument = swaggerDocument.swDocument;
    routes.filter(r => r.path !== undefined)
        .forEach(r => {
            if(!swaggerDocument.paths || !swaggerDocument.paths[r.path])
                swaggerDocument.paths[r.path] = {}

            const pathName = toUpFirstLetter(r.path.substring(1,r.path.length).split('/')[1]);
